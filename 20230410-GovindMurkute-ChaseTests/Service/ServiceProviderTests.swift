...
final class ServiceProviderTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        serviceProvider = ServiceProvider<WearhterService>()
    }
    ...
    func testLoadAPIShouldFail() {
        ...
        var tempData: [GeoLocationModel]?

        serviceProvider.load(type: [GeoLocationModel].self, service: .geoSearch(city: "")) { result in
            expectation.fulfill()
            switch result {
    ...
    func testLoadAPIShouldSuccess() {
        ...
        serviceProvider.load(type: [GeoLocationModel].self, service: .geoSearch(city: "pune")) { result in
            expectation.fulfill()
            switch result {
            case .success(let data, let response):
                if  (200...299).contains(response.statusCode) {
    ...
    func testWeatherDetailsAPIShouldSuccess() {
        let mockService = MockServiceProvider<WearhterService, WeatherInfo>(result: .success(mockWeatherDetailsSucccessResponseData() ?? WeatherInfo.init(id: nil, main: nil, name: nil, coord: nil, weather: nil, wind: nil), HTTPURLResponse()))
        let weatherInfoModel = WeatherInfoViewModel(provider: mockService)
        weatherInfoModel.fetchWeather {
            XCTAssertNotNil(weatherInfoModel.weatherInfo?.name)
            XCTAssertEqual(weatherInfoModel.weatherInfo?.name, "Mumbai")
        }
    }
    
    func testWeatherDetailsAPIShouldFail() {
        let mockService = MockServiceProvider<WearhterService, WeatherInfo>(result: .failure(NetworkError.serverError(statusCode: 401), HTTPURLResponse()))
        let weatherInfoModel = WeatherInfoViewModel(provider: mockService)
        weatherInfoModel.fetchWeather {
    ...
    func mockWeatherDetailsSucccessResponseData() -> WeatherInfo? {
        ...
        guard let model = try? JSONDecoder().decode(WeatherInfo.self, from: jsonString.data(using: .utf8) ?? Data()) else { return nil }
        return model
